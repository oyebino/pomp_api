...
class TestEditTraderPassword(BaseCase):
    ...
    def test_editTraderPwd(self,userLogin,send_data,expect):
        """修改密码"""
        re = Trader(userLogin).editTrader(send_data['name'],send_data['name'],send_data['tel'],send_data['parkName'],send_data['editPwd'])
        result = re.json()
    ...
    @pytest.mark.parametrize('weiXinLogin', [{'user': '${mytest.tel}', 'pwd': '${mytest.editPwd}'}], indirect=True)
