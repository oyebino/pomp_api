...
class TestTraderSell(BaseCase):
    ...
    def test_addTrader(self,userLogin,send_data,expect):
        """新增商户"""
        re = Trader(userLogin).addTrader(send_data["traderName"], send_data["parkName"], send_data['tel'],send_data['couponName'])
        result = re.json()
    ...
    def test_traderSell(self,userLogin,send_data,expect):
        """商家售卖"""
        re = Trader(userLogin).addSell(send_data['traderName'],send_data['couponName'])
        result = re.json()
        Assertions().assert_in_text(result, expect["traderSellMsg"])

    @pytest.mark.parametrize('weiXinLogin',[{'user':'${mytest.tel}','pwd':'123456'}],indirect=True)
