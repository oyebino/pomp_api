...
test_data,case_desc = YmlUtils("/test_data/parkingManage/businessCoupon_service/fixedAmountCoupon.yml").getData
@pytest.mark.parametrize(args_item, test_data)
@allure.feature("优惠劵管理")
class TestFixedAmountCoupon():
    """固定劵创建并使用"""
    ...
    def test_sentryPay(self,sentryLogin,send_data,expect):
        """岗亭端缴费"""
        re = CarInOutHandle().normal_car_out(send_data["parkUUid"])
        result = re.json()
