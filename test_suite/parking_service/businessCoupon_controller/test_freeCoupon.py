...
test_data,case_desc = YmlUtils("/test_data/parking_service/businessCoupon_controller/freeCoupon.yml").getData
@pytest.mark.parametrize(args_item, test_data)
@allure.feature("优惠劵管理")
class TestCouponFree():
    """免费券创建并使用"""
    def test_addCoupon(self,userLogin,send_data,expect):
        """新增优惠劵"""
        re = Coupon_controller(userLogin).addCoupon(send_data["couponName"])
        result = re.json()
