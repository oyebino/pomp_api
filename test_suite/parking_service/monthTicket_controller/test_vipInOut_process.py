...
test_data,case_desc = YmlUtils("/test_data/parking_service/monthTicket_controller/monthTicket_service.yml").getData
...
class Test_vip_in_out():
    ...
    #     re = monthTicket_controller(userLogin).renew_monthTicket(send_data["carNum"], send_data["ticketName"])
    #     result = re.json()
    #     Assertions().assert_in_text(result,expect["message3"])
    #
    # # 月票退款
    # def test_refund_monthTicket(self,userLogin,send_data,expect):
    #     re = monthTicket_controller(userLogin).refund_monthTicket(send_data["carNum"])
    #     result = re.json()
