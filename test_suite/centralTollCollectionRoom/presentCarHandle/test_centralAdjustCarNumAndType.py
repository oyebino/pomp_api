...
# @Author  : 叶永彬
# @File    : test_centralAdjustCarNumAndType
...
test_data,case_desc = YmlUtils("/test_data/centralTollCollectionRoom/presentCarHandle/centralAdjustCarNumAndType.yml").getData
@pytest.mark.parametrize(args_item, test_data)
@allure.feature("中央收费处")
@allure.story('中央校正车辆和类型')
class TestCentralAdjustCarNumAndType(BaseCase):
    """中央校正车辆和类型"""
    ...
    def test_centralAdjustCarNum(self, centralTollLogin, send_data, expect):
        """中央-校正在场车牌和类型"""
        re = PresentCarHandle(centralTollLogin).centralRecordsPath(send_data['carNum'], send_data['adjustCarNum'], send_data['adjustCarType'])
        result = re
        Assertions().assert_in_text(result, '')

    def test_centralPresentCarType(self, centralTollLogin, send_data, expect):
        """中央查看在场车辆类型"""
        re = PresentCarHandle(centralTollLogin).centerRecords(send_data['adjustCarNum'])
        result = re[0]
