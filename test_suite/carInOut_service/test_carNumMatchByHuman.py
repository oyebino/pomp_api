...
test_data,case_desc = YmlUtils("/test_data/carInOut_service/carNumMatchByHuman.yml").getData
...
class TestCarNumMatchByHuman(BaseCase):
    ...
    def test_presentCar(self,userLogin,send_data,expect):
        """查看在场记录"""
        re = Information_controller(userLogin).getPresentCar(send_data["parkId"],send_data["matchCarNum"])
    ...
    def test_matchCarNumByhuman(self,sentryLogin,send_data,expect):
        """人工匹配车牌"""
        re = CheckInOut(sentryLogin).match_carNum(send_data['parkUUID'],send_data['matchCarNum'])
    ...
    def test_carLeaveHistory(self,userLogin,send_data,expect):
        """查看离场记录"""
        re = Information_controller(userLogin).getCarLeaveHistory(send_data["parkId"],send_data["matchCarNum"])
        result = re.json()["data"]["rows"]
